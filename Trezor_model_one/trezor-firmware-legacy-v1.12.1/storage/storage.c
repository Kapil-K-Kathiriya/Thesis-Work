...
static secbool set_pin(const uint8_t *pin, size_t pin_len,
  ...
  uint8_t buffer[RANDOM_SALT_SIZE + KEYS_SIZE + POLY1305_TAG_SIZE] = {0}; //Make buffer for storing salt,ekeys,and pvc
  uint8_t *rand_salt = buffer;
  uint8_t *ekeys = buffer + RANDOM_SALT_SIZE;//KEY_SIZE = DEK + SAK(256bit+128bit)
  uint8_t *pvc = buffer + RANDOM_SALT_SIZE + KEYS_SIZE;

  uint8_t kek[SHA256_DIGEST_LENGTH] = {0};//key encryption key array to store kek
  uint8_t keiv[SHA256_DIGEST_LENGTH] = {0};//key encrytion Initialization vector array to store keiv
  chacha20poly1305_ctx ctx = {0}; //is a composite structure that combines the context data for both the ChaCha20 stream cipher and the Poly1305 MAC algorithm
  random_buffer(rand_salt, RANDOM_SALT_SIZE); //random salt is generated and stored in buffer, address start from rand_salt
...
static secbool decrypt_dek(const uint8_t *kek, const uint8_t *keiv) {
  ...
  if (sectrue != initialized ||
      ...
      len != RANDOM_SALT_SIZE + KEYS_SIZE + PVC_SIZE) { //KEY_SIZE = DEK + SAK(256bit+128bit)
    handle_fault("no EDEK");
    return secfalse;
  }

  const uint8_t *ekeys = (const uint8_t *)buffer + RANDOM_SALT_SIZE; //point ekeys which we get from storage
  const uint32_t *pvc = (const uint32_t *)buffer +
                        (RANDOM_SALT_SIZE + KEYS_SIZE) / sizeof(uint32_t); //point PVC which we get from storage
  _Static_assert(((RANDOM_SALT_SIZE + KEYS_SIZE) & 3) == 0, "PVC unaligned");
  _Static_assert((PVC_SIZE & 3) == 0, "PVC size unaligned");

  uint8_t keys[KEYS_SIZE] = {0}; // stored decrypted key(size=256+128)
  uint8_t tag[POLY1305_TAG_SIZE] __attribute__((aligned(sizeof(uint32_t))));//stored tag (size=64bit)
  ...
  chacha20poly1305_decrypt(&ctx, ekeys, keys, KEYS_SIZE); //output is keys //keys contain decrypted key and ekeys contain encrypted key
  rfc7539_finish(&ctx, 0, KEYS_SIZE, tag); //output is tag //at the end we get tag which is generated while decrypting the keys
  memzero(&ctx, sizeof(ctx));
  wait_random();
  if (secequal32(tag, pvc, PVC_SIZE) != sectrue) {//Now we will compare this tag and PVC are equal or not if not then failure otherwise success.
...
static secbool unlock(const uint8_t *pin, size_t pin_len,
  ...
  // KEIV from the PIN.
  const void *rand_salt = NULL; //Get rand_salt from memory, which is already stored inside function
  uint16_t len = 0;
  if (sectrue != initialized ||
      sectrue != norcow_get(EDEK_PVC_KEY, &rand_salt, &len) ||//this function is used to get rand_salt from memory
